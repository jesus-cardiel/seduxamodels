...
document.getElementById('form').addEventListener('submit', async (e) => {
    ...
    submit.disabled = true;
    showMsg('Enviando registro...', 'info');
    ...
    try {
        ...
        if (response.ok) {
            notifySuccess('¡Éxito!', 'Registro enviado para revisión correctamente.').then(() => {
                location.href = '/';
            });
            ...
            btnSelfie.classList.remove('ok');
            selfieHint.textContent = 'Se solicitará permiso de cámara.';
            // Trigger input events to disable fields again
            realName.dispatchEvent(new Event('input'));
        } else {
            notifyError('Error', data.message || 'Error al enviar el registro.');
            showMsg(data.message || 'Error al enviar el registro.', 'err');
            submit.disabled = false;
        }
    } catch (error) {
        notifyError('Error', 'Error de conexión con el servidor.');
