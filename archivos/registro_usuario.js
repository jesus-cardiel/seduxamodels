...
*/
const API_REGISTER = '/api/v1/vip/register';  // POST registro correo
const REDIRECT_VIP = '/vip/';                 // a dónde manda si registra OK
...
/* Resets dependientes sin tocar el nick */
...
formVip.addEventListener('submit', async (e) => {
  ...
  if (!payload.nick || !payload.edad || !payload.sexo || !payload.country_code || payload.whatsapp.length < 6 || !payload.email || payload.password.length < 6) {
    showMsg('Completa todos los campos obligatorios.', 'err');
    return;
  }
  ...
  try {
    ...
    setTimeout(() => location.href = REDIRECT_VIP, 400);
